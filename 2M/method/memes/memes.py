...
docking_count = 1
access_cnt = 0
all_scores = load_data("./../../data/smile_score_dict.dat")
...
#     smiles = f.readlines()
#     smiles = [i.strip('\n') for i in smiles]
smiles = load_data("./../../data/smile_list.dat")
...
with open(directory_path+'/start_smiles.txt','w') as f:
    for index in X_index:
        f.write(str(ligands[index]) + ',' + str(all_scores[smiles[index]]) + '\n')
...
save_data(result_list, f"./../../result/4UNN/{total_count}/memes_{af}_result{result_tail}.dat")